...
const webWorkItems: WorkItem[] = [
  ...
  {
    title: "電気工事会社様 HP制作",
    description: "東京都府中市で活動されている電気工事会社様のホームページを制作しました。",
    tags: ["Studio", "Google Analytics", "Google Search Console"],
  },
];

const podcastWorkItems: WorkItem[] = [
  {
    title: "ヒガコウカシタ ラジオ・レラ",
    description: "東小金井発、起業・開業にチャレンジする人を応援するポッドキャスト。小金井市特定創業支援等事業の実践型ワークショップ「コウカシタLAB」から始まりました。",
    link: "/links/higakoukashita-radio-rera",
  },
  {
    title: "ぶんじブリッジ",
    description: "国分寺で暮らす人々をつなぐ情報発信コミュニティ。国分寺市協業事業の連続講座「こくぶんじカレッジ」から生まれたポッドキャストです。",
    link: "https://linktr.ee/bunji_bridge",
  },
];
    ...
    <main>
      ...
      <section id="top" className="py-12 sm:py-16 md:py-20">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <h2 className="text-xl sm:text-2xl font-bold text-gray-800 mb-6 sm:mb-8">Web制作実績</h2>
          {webWorkItems.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 md:gap-8">
              {webWorkItems.map((item, index) => (
          ...
          <h2 className="text-xl sm:text-2xl font-bold text-gray-800 mb-6 sm:mb-8 mt-12 sm:mt-16">ポッドキャスト運営、支援実績</h2>
          {podcastWorkItems.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 md:gap-8">
              {podcastWorkItems.map((item, index) => (
                <WorkCard key={index} {...item} />
              ))}
            </div>
          ) : (
